...
exports.createProject = async (req, res) => {
  ...
  try {
    const newProject = new Project({ projectName, projectDesc, dueDated,userCreated });
...
exports.getUserProject = async (req,res) => {
	...
	try {
		const project = await User.findById(userId).populate('projects.own').populate('projects.membered')
		if(!project){
...
exports.deleteProject = async (req, res) => {
  ...
  try {
	...
	const usersUpdate = await User.find({
		$or: [
			{"projects.own":projectId},
			{"projects.membered": projectId},
		]
	});

	await Promise.all(usersUpdate.map(async(user)=>{
		user.projects.own = user.projects.own.filter(id=>id.toString() !== projectId)
		user.projects.membered = user.projects.membered.filter(id => id.toString() !== projectId);
